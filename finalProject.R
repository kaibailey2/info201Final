...
# Function to process data
  ...
  # Join shapefile for census tract map
  geoJSON <- st_read("Census_Tracts_2010.geojson")
  geoJSON$GEOID10 <- as.numeric(geoJSON$GEOID10)
  
  df <- merge(geoJSON, df, by.x = "GEOID10", by.y = "Census_Tract")
  
  # Remove non-Seattle census tracts
  df <- subset(df, !is.na(TRACT) & TRACT != "")
  ...
  # people spending above the 30% income threshold on housing, 1 being low severity, 5 being extreme severity).
  ...
  breaks <- quantile(df$Percentage, probs = seq(0, 1, 0.2), na.rm = TRUE)
  df$Ranking <- cut(df$Percentage, breaks, labels = c(1, 2, 3, 4, 5))
  ...
  saveRDS(summary_table, file = "summary.rds")
